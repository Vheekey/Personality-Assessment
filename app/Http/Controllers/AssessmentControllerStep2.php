...
class AssessmentControllerStep2 extends Controller
{
    public function index(Steps $steps)
    {
        $step = $steps->step('assessment.register', 2);
    ...
    {
        $steps->step('assessment.register', 2)
            ->store($request->all())
            ->complete();

        return redirect()->route('assessment.register.3.index', 'page=3');
    }
}
